...
CurrencyTable CurrencyConvertor::currencyTable = createCurrencyTable();

CurrencyTable CurrencyConvertor::createCurrencyTable()
{
    std::string currencyNA = "";
    std::string alphaNA = "";
    auto minorUnitNA = std::numeric_limits<int>::lowest();
    auto currencyCodeNA = std::numeric_limits<int>::lowest();

    return {{"AFGHANISTAN", "Afghani", "AFN", 971, 2},
            ...
            {"ANTARCTICA", currencyNA, alphaNA, currencyCodeNA, minorUnitNA},
            ...
            {"INTERNATIONAL MONETARY FUND (IMF) ", "SDR (Special Drawing Right)", "XDR", 960, minorUnitNA},
            ...
            {"MEMBER COUNTRIES OF THE AFRICAN DEVELOPMENT BANK GROUP", "ADB Unit of Account", "XUA", 965, minorUnitNA},
            ...
            {"PALESTINE, STATE OF", currencyNA, alphaNA, currencyCodeNA, minorUnitNA},
            ...
            {"SISTEMA UNITARIO DE COMPENSACION REGIONAL DE PAGOS \"SUCRE\"", "Sucre", "XSU", 994, minorUnitNA},
            ...
            {"SOUTH GEORGIA AND THE SOUTH SANDWICH ISLANDS", currencyNA, alphaNA, currencyCodeNA, minorUnitNA},
            ...
            {"ZZ01_Bond Markets Unit European_EURCO", "Bond Markets Unit European Composite Unit (EURCO)", "XBA", 955, minorUnitNA},
            {"ZZ02_Bond Markets Unit European_EMU-6", "Bond Markets Unit European Monetary Unit (E.M.U.-6)", "XBB", 956, minorUnitNA},
            {"ZZ03_Bond Markets Unit European_EUA-9", "Bond Markets Unit European Unit of Account 9(E.U.A.-9)", "XBC", 957, minorUnitNA},
            {"ZZ04_Bond Markets Unit European_EUA-17", "Bond Markets Unit European Unit of Account 17(E.U.A.-17)", "XBD", 958, minorUnitNA},
            {"ZZ06_Testing_Code", "Codes specifically reserved for testing purposes", "XTS", 963, minorUnitNA},
            {"ZZ07_No_Currency", "The codes assigned for transactions where no currency is involved", "XXX", 999, minorUnitNA},
            {"ZZ08_Gold", "Gold", "XAU", 959, minorUnitNA},
            {"ZZ09_Palladium", "Palladium", "XPD", 964, minorUnitNA},
            {"ZZ10_Platinum", "Platinum", "XPT", 962, minorUnitNA},
            {"ZZ11_Silver", "Silver", "XAG", 961, minorUnitNA}};
}

Currency CurrencyConvertor::getCurrency(std::string alpha)
{
    CurrencyTable::const_iterator it = find_if(currencyTable.begin(), currencyTable.end(), [&](Currency currentCurrency) -> bool
                                               { return alpha == currentCurrency.getAlpha(); });
    if (it != currencyTable.end())
        return *it;
    return Currency();
}

Currency CurrencyConvertor::getCurrency(int currencyCode)
{
    CurrencyTable::const_iterator it = find_if(currencyTable.begin(), currencyTable.end(), [&](Currency currentCurrency) -> bool
                                               { return currencyCode == currentCurrency.getCurrencyCode(); });
    if (it != currencyTable.end())
        return *it;
    return Currency();
}
