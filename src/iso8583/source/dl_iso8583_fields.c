...
#include "iso8583/include/dl_iso8583_fields.h"
						  ...
						  DL_UINT8                   **ioPtr )
{
    ...
    const DL_ISO8583_MSG_FIELD *fieldPtr      = ((const DL_ISO8583_MSG*)iMsg)->field + iField;
						   ...
						   DL_UINT8                   **ioPtr )
{
	...
	if ( !err )
	{
        DL_MEM_memcpy((void*)tmpDataPtr,tmpPtr,size);
		tmpPtr     += size;
		tmpDataPtr += size;
...
{
	DL_ERR    err    = kDL_ERR_NONE;
    const DL_UINT8 *tmpPtr = (const DL_UINT8*)*ioPtr;
