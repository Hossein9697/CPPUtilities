...
#include "iso8583/include/dl_iso8583.h"
			                 ...
			                 DL_UINT16                *oNumBytes )
{
	...
	*oNumBytes = 0;
	...
	if ( !err )
	{
        ...
        *oNumBytes = (DL_UINT16)(curPtr - ioByteArr);
	}
...
int DL_ISO8583_MSG_Unpack( const DL_ISO8583_HANDLER *iHandler,
                               DL_UINT8           *iByteArr,
			                   DL_UINT16                 iByteArrSize,
			                   DL_ISO8583_MSG           *ioMsg )
{
	DL_ERR     err         = kDL_ERR_NONE;
    DL_UINT8  *curPtr      = iByteArr;
