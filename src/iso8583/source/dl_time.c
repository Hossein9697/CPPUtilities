...
#include "iso8583/include/dl_time.h"
...
static int timestamp_to_struct (char *iTimestampStr,
                                 DL_TIME    *oData );
...
DL_UINT32 DL_TIME_GetUTCSeconds ( void )
{
    ...
    ret = ((unsigned long)time(NULL) & DL_MAX_UINT32);
										   ...
										   DL_TIME   *oData )
{
    ...
    iUtcSecs += (unsigned int)TZ_INFO_BASE_BIAS(&_tz);
	...
	if ( TZ_INFO_HAS_DST(&_tz) )
	{
		...
		{
			/* apply DST bias */
            iUtcSecs += (unsigned int)TZ_INFO_DST_BIAS(&_tz);
		}
...
int DL_TIME_ConvUTCTimestampToUTCSeconds ( char  iTimestamp[],
										   DL_UINT32  *oUtcSecs )
{
	int     ok = 1;
	DL_TIME st;
...
int DL_TIME_ConvLocalTimestampToUTCSeconds ( char  iTimestamp[],
											 DL_UINT32  *oUtcSecs )
{
	int     ok = 1;
	DL_TIME st;
	...
	*oUtcSecs = 0;
	...
	if ( ok )
	{
		...
		if ( TZ_INFO_HAS_DST(&_tz) )
		{
            ...
            dstStart += (unsigned int)TZ_INFO_DST_BIAS(&_tz);
            dstEnd   -= (unsigned int)TZ_INFO_DST_BIAS(&_tz);
			...
			{
				/* remove DST bias */
                *oUtcSecs -= (unsigned int)TZ_INFO_DST_BIAS(&_tz);
			}
		}

		/* remove base Bias to UTC value */
        *oUtcSecs -= (unsigned int)TZ_INFO_BASE_BIAS(&_tz);
	}
							    ...
							    DL_TIME   *oData )
{
    ...
    for ( i=0,done=0 ; (i<12) && (!done) ; i++ )
    {
        ...
        tempUInt32 = (DL_UINT32)(calc_days_in_month(year,month) * kSECONDS_IN_DAY);
    ...
    iSeconds -= (unsigned int)(day-1)  * kSECONDS_IN_DAY;

    /* calculate 'hours' (0-n) */
    hour       = iSeconds / kSECONDS_IN_HOUR;
    iSeconds  -= (unsigned int)hour     * kSECONDS_IN_HOUR;

    /* calculate 'minutes' (0-n) */
    minute    = iSeconds / kSECONDS_IN_MINUTE;
    iSeconds -= (unsigned int)minute   * kSECONDS_IN_MINUTE;
							    ...
							    DL_UINT32     *oSeconds )
{
	...
	*oSeconds = 0;
	...
	for ( i=1 ; i<tmpSt.month ; i++ )
        *oSeconds += (unsigned int)calc_days_in_month(tmpSt.year,i) * kSECONDS_IN_DAY;

	/* process 'day' */
    *oSeconds += (unsigned int) (tmpSt.day - 1) * kSECONDS_IN_DAY;

	/* process 'hours' */
    *oSeconds += (unsigned int) tmpSt.hour * kSECONDS_IN_HOUR;

	/* process 'minutes' */
    *oSeconds += (unsigned int) tmpSt.minute * kSECONDS_IN_MINUTE;

	/* process 'seconds' */
    *oSeconds += (unsigned int) tmpSt.second;
								 ...
								 DL_TIME    *oData )
{
    ...
    if ( ok && ((DL_STR_StrCmp((DL_CHAR*)iTimestampStr,(DL_CHAR*)kTIMESTAMP_MIN_VALUE,0) == -1) ||
                (DL_STR_StrCmp((DL_CHAR*)iTimestampStr,(DL_CHAR*)kTIMESTAMP_MAX_VALUE,0) ==  1)) )
	{
		ok = 0;
	}
                              ...
                              int iDay )
{
    ...
    for ( i=1 ; i<iMonth ; i++ )
    {
        numDays += (DL_UINT32)calc_days_in_month(iYear,i);
    }

    /* calculate days for month days */
    if ( iDay > 1 )
    {
        numDays += (DL_UINT32)(iDay - 1);
    }
